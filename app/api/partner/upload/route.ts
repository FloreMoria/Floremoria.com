...
export async function POST(req: NextRequest) {
    try {
        ...
        if (order) {
            ...
            // Protocollo Omniscienza: Upsert DeliveryProof con le prove e le note
            await prisma.deliveryProof.upsert({
                where: { orderId: order.id },
                create: {
                    orderId: order.id,
                    partnerId: order.partnerId || 'default-partner', // Fallback se null
                    photoBeforeUrl: photoPaths[0] || null,
                    photoAfterUrl: photoPaths[1] || photoPaths[0] || null,
