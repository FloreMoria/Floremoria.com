...
export async function GET(request: Request, ctx: { params: Promise<{ agencyId: string }> }) {
    const auth = await authenticatePartnerV1(request);
    if (!auth) return unauthorized(request);

    const { agencyId: rawAgency } = await ctx.params;
    const agencyId = decodeURIComponent(rawAgency || '').trim();
    if (!agencyId) {
        return NextResponse.json(
            { error: 'agencyId non valido.' },
            { status: 400, headers: { ...partnerV1CorsHeaders(request, 'GET, OPTIONS') } }
        );
    }
