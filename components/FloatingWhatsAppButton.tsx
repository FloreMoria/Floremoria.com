...
const WA_PREFILL = encodeURIComponent(
    'Salve FloreMoria, vorrei ricevere informazioni e assistenza'
);

/** Prefissi PDP con sticky CTA "Ordina" sopra la bottom nav. */
...
function isProductDetailPath(pathname: string | null): boolean {
    ...
    return PRODUCT_PATH_PREFIXES.some((prefix) => pathname.startsWith(prefix) && pathname.length > prefix.length);
}

/**
 * Pulsante flottante assistenza WhatsApp — visibile anche su mobile sopra bottom nav / sticky Ordina.
 */
export default function FloatingWhatsAppButton() {
    const pathname = usePathname();
    const onProductPage = isProductDetailPath(pathname);
    const href = `https://wa.me/${WA_PHONE}?text=${WA_PREFILL}`;

    // Ordinario: sopra MobileBottomNav (~3.25rem). PDP: sopra sticky Ordina (z-998) + nav.
    ...
    return (
        <a
            ...
            className={`fixed right-4 ${bottomClass} z-[999] inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_10px_24px_rgba(37,211,102,0.35)] transition-transform duration-200 hover:scale-105 hover:shadow-[0_14px_28px_rgba(37,211,102,0.42)] focus:outline-none focus:ring-2 focus:ring-[#25D366]/60 focus:ring-offset-2 md:right-6 md:bottom-auto md:top-24`}
        >
            {/* Micro-pulse discreto — solo anello, non invasivo */}
            <span
                className="pointer-events-none absolute inset-0 rounded-full bg-[#25D366]/40 animate-ping opacity-40 md:opacity-0"
                aria-hidden="true"
            />
            <span className="relative flex items-center justify-center">
                ...
                <span className="absolute -right-0.5 -top-0.5 h-3 w-3 rounded-full border-2 border-white bg-emerald-400 md:hidden" aria-hidden="true" />
            </span>
        </a>
    );
}
