...
export default function CoreValues({ className = '' }: { className?: string }) {
    const values = [
        {
    ...
    return (
        <section className={`w-full max-w-6xl mx-auto px-4 py-3 sm:py-6 ${className}`}>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                {values.map((item, idx) => (
                    <div
                        key={idx}
                        className="flex flex-col items-center text-center p-3 sm:p-4 md:p-6 bg-white border border-gray-100 rounded-xl sm:rounded-2xl shadow-sm hover:shadow-md transition-shadow"
                    >
                        <div className="mb-2 sm:mb-3 md:mb-4">
                            {item.icon}
                        </div>
                        <h4 className="font-display font-bold text-gray-900 text-[14px] sm:text-[15px] md:text-lg mb-1 md:mb-2 leading-tight">
                            {item.title}
                        </h4>
                        <p className="text-gray-500 font-body text-[11px] sm:text-[12px] md:text-[14px] leading-snug">
                            {item.description}
                        </p>
                    </div>
                ))}
