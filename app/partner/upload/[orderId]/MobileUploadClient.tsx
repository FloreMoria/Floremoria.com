...
export default function MobileUploadClient({ orderId, isFuneral }: { orderId: string, isFuneral: boolean }) {
  ...
  return (
    <div className="max-w-md mx-auto relative pt-12 px-6">
      ...
      {/* NOTE OPERATIVE */}
      <div className="mt-6 bg-white rounded-[24px] p-6 shadow-sm border border-[#EAE3D9]">
        <h3 className="font-semibold text-[15px] text-[#111B21] mb-2">
          Note Tecniche sulla Consegna (Facoltativo)
        </h3>
        <p className="text-[13px] text-[#54656F] mb-4">
          Indica informazioni utili (es. 'dietro la cappella grande') per facilitare le prossime consegne.
        </p>
        <textarea
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          disabled={status !== 'idle' && status !== 'error'}
          placeholder="Scrivi qui le tue note tecniche..."
          className="w-full bg-[#F0F2F5] border-none rounded-xl p-4 text-[15px] text-[#111B21] focus:ring-2 focus:ring-[#1DA851]/50 transition-shadow resize-none"
          rows={3}
        />
      </div>
