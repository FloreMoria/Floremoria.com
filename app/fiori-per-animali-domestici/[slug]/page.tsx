...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
    ...
    return {
        title: `${product.name} - Piccoli Amici | FloreMoria`,
        description: `Pianta e omaggio per piccoli amici consegnato da fiorista locale con foto su WhatsApp`,
        alternates: {
...
export default async function PetProductPage({ params, searchParams }: ProductPageProps) {
    const resolvedParams = await params;
    ...
    const nativeUrl = getProductUrl(product);
    if (!nativeUrl.startsWith('/fiori-per-animali-domestici/')) {
