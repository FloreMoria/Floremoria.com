...
type BankStatementsPanelProps = {
    /** `tab1`: upload only + archivio collassabile; nasconde la tabella movimenti grande. */
    variant?: 'full' | 'tab1';
};
...
export default function BankStatementsPanel({ variant = 'full' }: BankStatementsPanelProps) {
    ...
    const runPastePreview = async () => {
        ...
        try {
            ...
            setPasteRows(parsed.data?.rows || []);
            const out = parsed.data?.outOfPeriodCount ?? 0;
            setPasteSummary(
                (parsed.data?.parseSummary ||
                    `${parsed.data?.newCount ?? 0} nuovi · ${parsed.data?.duplicateCount ?? 0} già presenti`) +
                    (parsed.data?.openPeriodLabel
                        ? ` · periodo aperto ${parsed.data.openPeriodLabel}`
                        : '') +
                    (out > 0 ? ` · ${out} fuori periodo` : '')
            );
        } catch (e) {
    ...
    const runPasteConfirm = async () => {
        ...
        if (newCount <= 0) {
            setPasteError('Nessun movimento nuovo da salvare nel periodo aperto.');
            return;
        }
        setPasteSaving(true);
        setPasteError(null);
            ...
            >
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                    <div className="flex items-center gap-3 text-sm text-slate-600">
                        ...
                        <span>
                            Periodi chiusi: solo file ufficiale dal portale (PDF/CSV/Excel) con
                            saldi dichiarati. Periodo aperto: puoi anche incollare la lista
                            movimenti (stato <strong>provvisorio</strong>, METODO §2).
                        </span>
                    </div>
                    <div className="flex flex-wrap items-center justify-end gap-2 shrink-0">
                        ...
                        >
                            ...
                            Lista Movimenti (periodo aperto)
                        </button>
                        <button
                            type="button"
                            disabled={uploading}
                        ...
                        >
                            ...
                            {uploading ? 'Elaborazione…' : 'Carica Estratto Ufficiale'}
                        </button>
                    </div>
                    <input
                        ref={inputRef}
                    ...
                    <div className="w-full sm:max-w-3xl max-h-[92vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl bg-white shadow-xl border border-slate-100">
                        <div className="sticky top-0 flex items-center justify-between gap-2 px-4 py-3 border-b border-slate-100 bg-white z-10">
                            <div>
                                ...
                                >
                                    Lista Movimenti Fineco (provvisorio)
                                </h3>
                                <p className="text-[11px] text-slate-500 mt-0.5">
                                    Solo periodo aperto. Saldi/intestazioni scartati. Dedup su
                                    fingerprint. All&apos;arrivo dell&apos;estratto ufficiale le
                                    righe passano a certificate o in Eccezioni.
                                </p>
                            </div>
                            <button
                                type="button"
                                ...
                                <p className="text-xs text-slate-600 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2">
                                    ...
                                    {pasteRows.length > 0 && (
                                        <span className="text-slate-400">
                                            {' '}
                                            · {pasteNewCount} nuovi /{' '}
                                            {pasteRows.length - pasteNewCount} scartati o già presenti
                                        </span>
                                    )}
                                </p>
                            )}
